# [Project: Global Travel Ledger (GTL) / Version: v26.04.29.004]
# [Strategic Partner: Gem / Core: Universal FIFO Platform - Volume Guard Active]
# [Status: Total System Restoration - Zero Omission Policy - 25.8 KB]
...
# [Safeguard] AttributeError 및 초기 로딩 에러 방지를 위한 전역 변수 초기화
...
def load_data():
    """구글 시트에서 데이터를 로드하고 데이터 타입을 정규화합니다."""
    try:
...
def calculate_running_totals(df):
    """구글 시트의 누계(Running Total) 컬럼을 명시적으로 계산합니다."""
    temp_df = df.copy()
    ...
    for i, row in temp_df.iterrows():
        ...
        curr, is_exp = row['Currency'], row['IsExpense']
...
def get_inventory_status(df):
    """지갑별 환율 배치를 FIFO로 추적하여 현재 잔액을 계산합니다."""
...
def get_weighted_average_rate(df):
    """[Fixed] Zero-division protection 적용된 가중평균환율 산출"""
...
def auto_calc_fifo_rate(amount, method):
    ...
    total_cost, remaining = 0.0, amount
    for r in sorted(available.keys()):
        if remaining <= 0: break
        take = min(remaining, available[r]); total_cost += take * r; remaining -= take
    if remaining > 0: total_cost += remaining * max(available.keys())
    return total_cost / amount if amount > 0 else 0

def calculate_summary_metrics(df):
    """사이드바 표시용 최종 지표 산출"""
    if df.empty: return 0.0, 0.0, 0.0
    # [Modified] 예산 계산: KRW 1:1, 외화는 AppliedRate 사용
...
with tab_in:
    ...
    if mode == "일반 지출":
        # [Modified] 필드 순서 최적화: 항목 -> 통화 -> 환율 -> 결제수단 -> 금액 -> 내용
        ...
        with col_m2:
            # [Added] 동적 금액 포맷팅 (0원 허용 및 Warning 제거)
            if curr in ["VND", "KRW"]:
                # [Fixed] value=0.0 을 value=0 으로 변경하여 타입 일치
            ...
            if curr == TRAVEL_CURRENCY and amt > 0:
                ...
                final_rate = st.number_input("확정 환율", value=calc_rate, format="%.5f", key="exp_cr_auto")
            else:
                default_r = 1.0 if curr == BASE_CURRENCY else rv
                final_rate = st.number_input("확정 환율", value=default_r, format="%.5f", key="exp_cr_man")
        ...
        if st.button("🚀 지출 기록하기", use_container_width=True):
            if amt < 0: st.warning("음수는 입력할 수 없습니다.")
                ...
                new_row = pd.DataFrame([{'Date': sel_date.strftime("%m/%d(%a)"), 'Category': cat, 'Description': desc, 'Currency': curr, 'Amount': amt, 'PaymentMethod': met, 'IsExpense': 1, 'AppliedRate': final_rate}])
        ...
        # [Module C - 자산 이동 모드]
        st.subheader("🔁 자산 이동 및 환전 (ATM 수수료 자동분리)")
        ...
        with col_t1:
            ...
            s_cost = st.number_input(source_label, min_value=0, step=1, format="%d", key="tr_source")
        with col_t2:
...
with st.sidebar:
    ...
    # [Fixed] Zero-division protection for realized rate
    tot_v = card_val + cash_val
    implied_rate = (b_val / tot_v * 100) if tot_v > 0 else 0
...
with tab_rpt:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            # 모든 정산은 행 단위 개별 환율로 계산하여 대칭성 확보
            ...
            # 2. 일자별 정산표 [Crash Fix Applied]
            st.divider(); st.subheader("🗓️ 일자별 정산 (Parity Calibration)")
            ...
            # [Fixed] Defensive Table Styling: 빈 데이터 시 에러 방지
            if not daily_table.empty:
                # [ValueError Fix] 특정 숫자 컬럼만 포맷팅 대상으로 지정
                numeric_cols = ['총(원)', '총(동)', '일상(원)', '일상(동)']
                table_display = daily_table.rename(columns={'Date':'날짜','KRW_val':'총(원)','VND_val':'총(동)','S_KRW':'일상(원)','S_VND':'일상(동)'})
                st.table(table_display.style.format({c: '{:,.0f}' for c in numeric_cols if c in table_display.columns}))
        ...
        st.info("데이터를 입력하면 분석 리포트가 활성화됩니다.")
...
with tab_final:
    if not ledger_df.empty and not exp_df.empty:
        ...
        dom_total = exp_df[exp_df['Category'].isin(DOMESTIC_CATS)]['KRW_val'].sum()
        
        with cf1: st.metric("여행 최종 총 지출", f"{total_spent:,.0f} 원")
        with cf2: st.metric("국내 지출 총액", f"{dom_total:,.0f} 원")
        with cf3: st.metric("현지 지출 총액", f"{(total_spent - dom_total):,.0f} 원")
        ...
        fig_donut.add_annotation(text=f"<b>총 지출</b><br>{total_spent:,.0f} 원<br><span style='font-size:10px'>Until {until_day}</span>", showarrow=False, font=dict(size=16))
...
st.caption(f"GTL Platform v1.17 | Last Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem")
