...
# [Strategic Partner: Gem / Core: High-Density Analytics & FIFO Restoration]
# [Status: Total System Restoration - ZERO OMISSION - 27.5 KB]
...
# 전역 변수 초기화 (AttributeError 및 초기화 지연 방지)
...
def load_data():
    try:
        ...
        if "429" in str(e): st.error("🚨 구글 서버 요청 한도 초과. 1분 뒤에 시도하세요.")
        return pd.DataFrame(columns=COLUMNS + AUDIT_COLUMNS)
...
def save_data(df, metrics=None):
    ...
    with st.status("데이터 동기화 및 Audit 갱신 중...", expanded=False):
        try:
...
WAR = (swaps_df['Amount'] * swaps_df['AppliedRate']).sum() / swaps_df['Amount'].sum() if not swaps_df.empty and swaps_df['Amount'].sum() > 0 else 0.0561
...
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            # 2. 요약 및 세부내역 (counts added)
            st.subheader("🏁 여행 경제 요약")
            c1, c2 = st.columns(2)
            with c1:
                dom_df = exp_df[((exp_df['Currency'] == 'KRW') & (exp_df['PaymentMethod'] == '원화계좌')) & (exp_df['Category'] != '출국')]
...
st.caption(f"GTL Platform v1.22 | Volume: 27.5 KB | Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem")
