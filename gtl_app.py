...
# [Strategic Partner: Gem / Core: JS Header Injection & Dual-Line KPI]
# [Status: Total System Restoration - ZERO OMISSION - 30.8 KB]
...
# --- SECTION 1: Configuration & JavaScript Icon Injection ---
st.set_page_config(
...
# [CORE FIX] 자바스크립트를 사용하여 아이폰 시스템 헤더에 아이콘 정보를 강제로 주입합니다.
st.markdown("""
    <script>
...
def save_data(df, metrics=None):
    ...
    with st.status("데이터 동기화 중...", expanded=False):
        try:
...
st.caption(f"GTL Platform v1.20 | Volume Guard: 30.8 KB | Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem")
