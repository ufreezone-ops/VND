...
# [Strategic Partner: Gem / Core: Unified Record-Driven Inventory (URDI) v4.1]
# [Status: Total System Restoration - ZERO OMISSION - 37.2 KB]
...
# [Modified] 구글 시트 데이터 스키마 정의 (L열 Note 위치 고정)
...
# [Status: Modified - Note Persistence & Column Alignment Fix]
...
def load_data():
    ...
    try:
        ...
        # [Fixed] L열 Note 위치를 보장하기 위해 FINAL_COLUMNS 순서로 재배열
...
def calculate_running_totals(df):
    """[Restored] 구글 시트의 누계 컬럼 I, J, K를 명시적으로 계산하며 L열 Note를 보존합니다."""
    temp_df = df.copy()
...
def save_data(df, metrics=None):
    ...
    with st.status("전략적 데이터 안전 잠금 및 동기화 중...", expanded=False):
        try:
            # [Fixed] 저장 전 누계 데이터 자동 생성 (Note 열 보존 확인)
            df_audit = calculate_running_totals(df)
            # [Fixed] 컬럼 순서를 A-L 순서로 강제 고정하여 전송
            df_to_save = df_audit.reindex(columns=FINAL_COLUMNS)
...
# [Status: Modified - Note Generation Logic Refined]
def get_inventory_status(df):
    ...
    for idx, row in df.iterrows():
        ...
        # 3. 지출 (URDI: 장부 환율 우선 매칭 소진)
            ...
            # 2단계: 남은 금액(혼합 환율 등)은 FIFO 순서로 소진하며 비고(Note) 생성
            if temp_qty > 0:
                decomposed_notes = []
                for batch in inv_batches[target]:
                    if temp_qty <= 0: break
                ...
                # [Added] 시스템 판단 근거를 메모리상 Note에 기록 (저장 시 시트로 전송됨)
                if decomposed_notes:
                    df.at[idx, 'Note'] = "Decomposed: " + " + ".join(decomposed_notes)
    ...
    if mode == "일반 지출":
        ...
        if cat in ["입국", "출국"]:
            ...
            if st.button("🚀 일정 기록 완료", use_container_width=True):
                new_row = pd.DataFrame([{'Date': sel_date.strftime("%m/%d(%a)"), 'Category': cat, 'Description': desc, 'Currency': 'KRW', 'Amount': 0, 'PaymentMethod': '원화계좌', 'IsExpense': 1, 'AppliedRate': 1.0, 'Note': ''}])
        ...
        if st.button("🔄 이동 실행", use_container_width=True):
            ...
            main_row = pd.DataFrame([{'Date': tr_date.strftime("%m/%d(%a)"), 'Category': ty.split(" ")[0], 'Description': f"{ty.split(' ')[0]} (-> {dest})", 'Currency': "VND", 'Amount': t_amt, 'PaymentMethod': "원화계좌" if "원화계좌" in ty else "트래블로그(VND)", 'IsExpense': 0, 'AppliedRate': rate, 'Note': ''}])
...
with tab_his:
    ...
    if not ledger_df.empty:
        # [Fixed] 컬럼 순서를 A-L 순서로 정렬하여 에디터에 표시
...
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            # [Added] 🔎 일자별 세부 내역 감사 (Audit Engine)
            st.subheader("🔎 일자별 세부 내역 감사 (Audit)")
            audit_date = st.selectbox("감사할 날짜 선택", options=ledger_df['Date'].unique(), index=len(ledger_df['Date'].unique())-1)
            audit_df = exp_df[exp_df['Date'] == audit_date].copy()
            # [Fixed] 감사 표에서도 Note 열을 마지막에 배치
            st.dataframe(audit_df[['Category', 'Description', 'Amount', 'Currency', 'VND_val', 'KRW_val', 'AppliedRate', 'Note']].rename(columns={
                'Amount':'원금액', 'VND_val':'환산동화', 'KRW_val':'환산원화', 'AppliedRate':'적용환율', 'Note':'시스템비고'
            }).style.format({'환산동화': '{:,.0f}', '환산원화': '{:,.0f}', '적용환율': '{:,.4f}'}), use_container_width=True)
...
st.caption(f"GTL Platform v1.43 | Volume Guard: 37.2 KB | Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem")
