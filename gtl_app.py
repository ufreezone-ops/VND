...
# [Strategic Partner: Gem / Core: iOS App-Icon Injection & Reactive FIFO]
# [Status: Total System Restoration - ZERO OMISSION - 30.5 KB]
...
# --- SECTION 1: Configuration & iOS App-Icon Injection ---
st.set_page_config(page_title="GTL: 여행 가계부", page_icon="🌏", layout="wide")

# [Added] 아이폰 홈 화면 추가 시 전용 아이콘(연두색 지구본)을 강제로 지정합니다.
# 이 코드가 있어야 Safari '홈 화면에 추가' 시 푸꾸옥 앱과 다른 아이콘이 생성됩니다.
st.markdown("""
    <head>
        <link rel="apple-touch-icon" href="https://img.icons8.com/color/512/globe--v1.png">
    ...
    .kpi-value-vnd { font-size: 16px; color: #00FF00; margin-top: 6px; font-family: 'Courier New', monospace; font-weight: 500; }
...
def load_data():
    try:
        ...
        if "429" in str(e): st.error("🚨 구글 서버 API 요청 한도 초과. 1분 뒤에 새로고침하세요.")
...
# [Status: Modified for Fixed Reactive Rate Injection]
    ...
    if mode == "일반 지출":
        ...
        if cat in ["입국", "출국"]:
            ...
            with col_m2:
                ...
                # [CORE FIX: Reactive Rate Injection with Dynamic Key]
        ...
        # [Module C - 자산 이동: ATM Rate Inheritance]
        st.subheader("🔁 자산 이동 (ATM 환율 자동상속)")
...
st.caption(f"GTL Platform v1.28 | Volume Guard: 30.1 KB | Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem")
