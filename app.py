# [Project: Phu Quoc Strategic Ledger / Version: v26.04.22.009]
# [Modules A, B, C, E, F: Maintained] / [Module D: Fully Enhanced]
# Total Line Count: 1075
...
# [Strategy] 일상 지출(Survival) 정의: Grab 포함, 택시(한국) 제외
...
with tab_history:
    ...
    st.info("📱 셀 **두 번 터치**로 수정, 표 바깥 터치 후 **클라우드 저장** 버튼 필수.")
    if not ledger_df.empty:
        # [Modified] 정순 정렬 유지하여 에디터 인덱스 고정
        edited_df = st.data_editor(ledger_df, use_container_width=True, num_rows="dynamic", key="history_editor_v9")
        if not ledger_df.equals(edited_df): st.warning("⚠️ 수정된 내용이 있습니다! 아래 버튼을 누르세요.")
...
# --- [TAB 3: 일일 결산 및 환전 전략 (Module D: Finalized)] ---
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            # [Modified] Survival 필터: Grab 포함, 택시 제외
            ...
            st.subheader("🗓️ 일자별 정산 (VND 기준 이중 통화)")
            ...
            rem_days = max(0, (last_date - today).days + 1)
            ...
            total_on_hand = current_v + current_cash
            shortage = pred_need - total_on_hand
            ...
            # [Modified] 차트: 일상(Bar) vs 전체(Line) 혼합형
            st.divider()
            st.subheader("📈 지출 추이 분석 (4/20 ~ 4/27)")
            ...
            # [Added] 0값 NaN 처리하여 선 그래프 끊기 (미래 날짜 대응)
            ...
            fig.add_scatter(x=chart_final['Date'], y=chart_final['Total_VND_Disp'], name="이벤트 포함 전체지출", line=dict(color='#FF00FF', width=3), connectgaps=False)
            st.plotly_chart(fig, use_container_width=True)

            # [Added] 원형 그래프: 항목별 지출 비중
            st.divider()
            st.subheader("🍕 항목별 지출 비중 분석")
            cat_pie_df = exp_df.groupby('Category')['VND_val'].sum().reset_index()
            fig_pie = px.pie(cat_pie_df, values='VND_val', names='Category', hole=0.4, title="카테고리별 분포 (VND 환산 기준)", color_discrete_sequence=px.colors.qualitative.Pastel)
            fig_pie.update_traces(textposition='inside', textinfo='percent+label')
            st.plotly_chart(fig_pie, use_container_width=True)
        else: st.info("지출 내역이 없습니다.")
    else: st.info("데이터가 없습니다.")

st.caption(f"Last Sync: {datetime.now().strftime('%H:%M:%S')} | Strategic Partner Gem | v26.04.22.009")
