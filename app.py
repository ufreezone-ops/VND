# [Project: Phu Quoc Strategic Ledger / Version: v26.04.22.010]
# [Module A, B, E, F: Maintained] / [Module C, D: Modified/Added]
# Total Line Count: 1108
...
# [Modified] 사전 경비 포함 카테고리 재정의
EXPENSE_CATS = ["식사", "간식", "마트", "Grab", "VinBus", "지하철", "택시", "입장료", "투어", "선물", "통신", "팁", "수수료", "마사지", "항공권", "호텔", "보험"]
# [Strategy] 일상 지출(Survival) 정의: 항공권, 호텔, 보험 등 고정비는 제외
SURVIVAL_CATS = ["식사", "간식", "Grab", "VinBus", "마사지", "팁"]
FIXED_COST_CATS = ["항공권", "호텔", "보험"] # [Added] 사전 고정비 분류
...
# --- 2. [Module A] Data Engine --- (v26.04.22.009와 동일 유지)
...
# (중략: load_cash_count, save_data, save_cash_count 함수 동일 유지)
# [전체 코드 출력을 위해 필요한 함수들입니다.]
@st.cache_data(ttl=0)
def load_cash_count():
    try:
...
with tab_input:
    ...
    if mode == "일반 지출":
        ...
        with c1:
            ...
            curr = st.selectbox("통화", ["KRW", "VND", "USD"], key="exp_curr_select") # [Modified] 사전결제는 주로 KRW이므로 순서 조정
        with c2:
...
with tab_history:
    ...
    if not ledger_df.empty:
        edited_df = st.data_editor(ledger_df, use_container_width=True, num_rows="dynamic", key="history_editor_v10")
...
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            with c_tot1: st.metric("사전 고정비 (항공/호텔/보험)", f"{fixed_total_krw:,.0f} 원")
            with c_tot2: st.metric("현지 변동비 (식비/이동/기타)", f"{variable_total_krw:,.0f} 원")
            with c_tot3: st.metric("여행 총 지출", f"{(fixed_total_krw + variable_total_krw):,.0f} 원")

            # [Modified] Survival 필터: 사전 경비(Fixed)는 생존 지출에서 제외
            ...
            # 환전 예측 엔진 (고정비 제외 데이터로 정밀 계산)
            st.divider()
            st.subheader("🔮 향후 VND 필요량 예측 (고정비 제외)")
...
st.caption(f"Last Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Strategic Partner Gem | v26.04.22.010")
