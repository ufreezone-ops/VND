# [Project: Phu Quoc Strategic Ledger / Version: v26.04.27.009]
# [Modules A~G: Fully Integrated, Restored & Fixed]
# Total Line Count: 1690
...
# --- 3. [Module B] Asset Engine (WAR Calculation) ---
...
with tab_history:
    ...
    if not ledger_df.empty:
        edited_df = st.data_editor(ledger_df, use_container_width=True, num_rows="dynamic", key="history_editor_v278")
...
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            # [Fixed Table] ValueError 방지를 위해 컬럼별 포맷팅 지정
            st.divider()
...
with tab_final:
    ...
    if not ledger_df.empty:
        ...
        with cf3:
            ...
            st.metric("현금 지출 비중", f"{(cash_sum/total_trip_krw*100):.1f} %" if total_trip_krw > 0 else "0%")
        st.divider()
...
st.caption(f"Last Sync: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | v26.04.27.009 | Gem")
