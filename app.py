# [Project: Phu Quoc Strategic Ledger / Version: v26.04.22.007]
# [Modules A, B, E, F: Maintained] / [Module C, D: Modified/Added]
# Total Line Count: 1022
...
# [Modified] 'Grab' 추가 및 카테고리 순서 조정
EXPENSE_CATS = ["식사", "간식", "마트", "Grab", "VinBus", "지하철", "택시", "입장료", "투어", "선물", "통신", "팁", "수수료", "마사지"]
...
# --- 2. [Module A] Data Engine (Multi-Sheet) ---
...
with st.sidebar:
    ...
    with st.expander("💵 실물 지폐 정산기", expanded=True):
        st.caption("Cloud 수량 기반. 수정 후 저장 필수.")
        curr_p_counts = {}
        total_ph = 0
        for b in BILLS:
...
with tab_input:
    ...
    if mode == "일반 지출":
        ...
        c1, c2 = st.columns(2)
        with c1:
            amt = st.number_input("금액", min_value=0.0, format="%.2f", key="exp_amt")
        ...
        with c2:
            ...
            sel_r_str = st.selectbox("적용 환율", r_opts, index=st.session_state.last_rate_idx, key="exp_rate")
            ...
            met = st.selectbox("결제수단", ["트래블로그(VND)", "현금(VND)", "원화계좌", "현대카드(USD)"], key="exp_method")
        ...
        ty = st.selectbox("유형", ["직접환전 (원화계좌 -> 지폐VND)", "충전 (원화계좌 -> 카드VND)", "충전 (원화계좌 -> 카드USD)", "ATM출금 (카드VND -> 지폐VND)", "보증금 지불 (지폐VND -> 보증금)"])
        c1, c2 = st.columns(2)
...
# --- [TAB 2: 내역 조회 및 수정 (Module E)] ---
with tab_history:
    st.subheader("🔍 내역 조회 및 수정")
    st.info("📱 **아이폰 수정 팁**: 셀을 **두 번 빠르게 터치**하여 수정하고, 표 바깥을 터치한 뒤 저장 버튼을 누르세요.")
    if not ledger_df.empty:
        edited_df = st.data_editor(ledger_df, use_container_width=True, num_rows="dynamic", key="history_editor_v7")
        if not ledger_df.equals(edited_df): st.warning("⚠️ 수정된 내용이 있습니다! 아래 버튼을 눌러 저장하세요.")
        col_edit_save, col_edit_del = st.columns(2)
        with col_edit_save:
            if st.button("💾 수정사항 클라우드 저장", use_container_width=True, type="primary"):
                if save_data(edited_df): st.toast("동기화 완료! ✅"); time.sleep(0.5); st.rerun()
        with col_edit_del:
...
with tab_stats:
    if not ledger_df.empty:
        ...
        if not exp_df.empty:
            ...
            # [Modified] Grab 추가, 택시 제외 (Survival Spend 정의)
            ...
            last_date = datetime(2026, 4, 27); today = datetime.now()
            remaining_days = max(0, (last_date - today).days + 1)
            ...
            shortage = predicted_need - (current_v + current_cash)

            c_p1, c_p2, c_p3 = st.columns(3)
            with c_p1: st.metric("일상 일평균 지출", f"{avg_survival:,.0f} ₫")
            with c_p2: st.metric(f"남은 {remaining_days}일 필요량", f"{predicted_need:,.0f} ₫")
            ...
            fig.add_scatter(x=chart_final['Date'], y=chart_final['Survival_VND'], name="일상 지출(Grab포함)", line=dict(color='yellow', width=4))
            st.plotly_chart(fig, use_container_width=True)
        else: st.info("지출 내역이 없습니다.")
    else: st.info("데이터가 없습니다.")

st.caption(f"Last Sync: {datetime.now().strftime('%H:%M:%S')} | v26.04.22.007 | Partner Gem")
